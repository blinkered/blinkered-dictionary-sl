# Blinkered dictionary: Slovene

The Slovene word list, and the evidence for every word in it.

Built by [`blinkered-attestation`](https://github.com/blinkered/blinkered-attestation). The rule,
the evidence format and the reasoning live there; what lives here is Slovene.

**136,507 of 262,639 candidates proved (52.0%)**, across 17 independent
families, 16 of which a stranger could check by fetching.

## What is in this repository

```
sources.mjs        which collections attest Slovene, and why those
...
Candidates come from Blinkered's Slovene list, which lives in
[`blinkered-attestation/candidates/sl`](https://github.com/blinkered/blinkered-attestation/tree/main/candidates/sl).
...
## What is particular to Slovene

**No eBible translation, and almost no Tatoeba.** Slovene Tatoeba is 4,222 distinct candidates, so
the small families that carry most languages are missing here. What Slovene has instead is an
unusually large Wikisource (44 million tokens, two thirds the size of the Wikipedia), Leipzig's
2020 news and 2014 `.si` web, five Gutenberg texts, the Internet Archive's Slovene books, and a
harvest of 4,753 pages from twelve Slovene publishers.

**The Archive shelf held a lot that was not Slovene.** Its metadata files Serbian and Croatian
material under Slovenian: thirty-odd issues of the Belgrade computing magazine *Svet kompjutera*,
for one. Books were screened by counting function words that differ between Slovene and its
neighbours (`in`, `ki`, `tudi`, `kot` against `koji`, `što`, `kao`, `vrlo`); 94 of 429 books were
removed, and 3 of those would have passed the legibility floor. The list is
`archive-sl/rejected.tsv` in the shared cache.

In the shipped list, 35,123 words (25.7%) are also in Croatian's list as it stood when this was
built. That overlap is real shared South Slavic vocabulary (ALI, TAKO, DOBRO); every source here
is Slovene-language, so it is not contamination. English in the list is negligible: ARE and NAME,
both also Slovene word forms.

**Where the ceiling is.** 52.0% of a 262,639-word list, most of which is inflection (Slovene keeps
the dual). Of the 58,095 words one family short, 33,508 were seen only by the Archive and a
Wikimedia project and 21,285 only by Leipzig and a Wikimedia project. A Common Crawl family
(FineWeb-2) would reach the first group and was not fetched for want of disk. The Archive
downloader was stopped at 429 books for time. RTV Slovenija offers no sitemap or feed the harvest
can find and gave no pages.

Every letter of the Slovene alphabet spells some shipped word; the rarest is F, in 3,099.
